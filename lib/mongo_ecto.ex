defmodule Mongo.Ecto do
  ...
  def execute(meta, _query_meta, {:nocache, {function, query}}, params, opts) do
    ...
    # function |> IO.inspect(label: "execute function")
    # query |> IO.inspect(label: "execute query")
    # params |> IO.inspect(label: "execute params")
    # opts |> IO.inspect(label: "execute opts")
  ...
  def insert(adapter_meta, schema_meta, fields, on_conflict, returning, opts) do
    {operation, args, opts} = case NormalizedQuery.insert(schema_meta, fields, on_conflict, returning, opts) do
      {operation, args} -> {operation, args, opts}
      {operation, args, opts} -> {operation, args, opts}
    end

    # IO.inspect({operation, args}, label: "HERE")

    case Connection.query(adapter_meta, operation, args, opts) |> IO.inspect(label: "query result") do
      ...
      {:ok, %Mongo.UpdateResult{ upserted_ids: [upserted_id]}} ->
        if [:id] == returning do
          {:ok, [id: upserted_id]}
        else
          {:ok, []}
      ...
      {:error, error} ->
         ...
         handle_potential_conflict(error, on_conflict)
    end
  end

  defp handle_potential_conflict(%Mongo.WriteError{ write_errors: [%{"code" => 11_000, "errmsg" => msg}]}, on_conflict), do: constraint_error(msg, on_conflict)

  defp handle_potential_conflict(%Mongo.Error{code: 11_000, message: msg}, on_conflict), do: constraint_error(msg, on_conflict)

  # Let the error pass through
  defp handle_potential_conflict(error, _on_conflict), do: {:error, error}

  defp constraint_error(_msg, {:nothing, _, _}), do: {:ok, []}

  defp constraint_error(msg, _on_conflict), do: {:invalid, [unique: extract_index(msg)]}
  ...
  # def insert(repo, meta, params, {%Ecto.Query{} = query, values, conflict_targets}, _returning, opts) do


  #   raise "MUST SUPPORT"s

  #   # IO.inspect(query.updates, label: "query updates")
  ...
  def insert_all(repo, meta, _fields, params, {:nothing, _list, conflict_targets}, _returning, _placeholders, opts) do
    ...
    command =
      ...
      |> Enum.map(fn doc ->
        ...
        query = case conflict_targets do
          [] -> %{}
          conflict_targets -> conflict_targets |> Enum.map(fn target -> {target, doc[target]} end)
        end
  ...
  #where: [id: ^123]`, `params` will be
  # `[123]`.
  def insert_all(repo, meta, fields, params, {%Ecto.Query{} = query, values, conflict_targets}, returning, placeholders, opts) do
    ...
    # set_fields = opts |> Keyword.get(:on_conflict) |> Keyword.get(:set, [])


    # [doc] = params

    # doc = doc |> Keyword.values()

    # norm_query = NormalizedQuery.update_all(query, doc ++ list) |> IO.inspect(label: "normalised query")
    ...
    command =
      ...
      |> Enum.map(fn doc ->
        ...
        # IO.inspect(find_query, label: "find_query")
        ...
        doc = case Map.get(update, :"$inc") do
          nil -> doc
          increment_fields ->
        ...
        [
          query: case conflict_targets do
            conflict_targets when is_list(conflict_targets) -> conflict_targets |> Enum.map(fn target -> {target, doc[target]} end)
            conflict_target when is_atom(conflict_target) -> Keyword.put([], conflict_target, doc[conflict_target])
          end,

          update: [
            # "$set": set_fields |> normalise_id(),
            # "$setOnInsert": doc
            # |> Enum.filter(fn {k, _v} -> k not in set_fields end)
            # |> normalise_id()
