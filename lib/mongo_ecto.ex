defmodule Mongo.Ecto do
  ...
  def insert(adapter_meta, schema_meta, fields, on_conflict, returning, opts) do
    ...
    case apply(Connection, normalized_query.op, [adapter_meta, normalized_query, opts]) do
      {:ok,
       %Mongo.UpdateResult{
         acknowledged: true,
         matched_count: 0,
      ...
      {:ok, %{"_id" => id}} ->
        # TODO need to know whether insert or update took place in order to decide whether to return the ID
        {:ok, [id: id]}
    ...
    # case Connection.query(adapter_meta, operation, args, opts) do
  ...
  defp handle_potential_conflict(
         %Mongo.WriteError{write_errors: [%{"code" => 11_000, "errmsg" => msg}]},
         on_conflict
       ),
       do: constraint_error(msg, on_conflict)

  defp handle_potential_conflict(%Mongo.Error{code: 11_000, message: msg}, on_conflict),
    do: constraint_error(msg, on_conflict)
