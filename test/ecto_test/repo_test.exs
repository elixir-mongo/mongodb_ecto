defmodule Ecto.Integration.RepoTest do
  ...
  describe "upsert via insert" do
    ...
    # Fails because not_inserted.id is autogenerated
    @tag :with_conflict_target
    ...
    test "on conflict ignore and conflict target" do
      ...
      # Not sure if this can be fixed because id is autogenerated and therefore
      # always returned regardless of whether the entity was created or not. 2021-09-01 JP.
      assert not_inserted.id == nil
    end
    ...
    @tag :with_conflict_target
    # TODO this needs to support multi!
    @tag :skip
  ...
  describe "upsert via insert_all" do
    ...
    @tag :with_conflict_target
    # Not sure if we can ever do this with MongoDB since we can't replace an _id
    # field (the only way is to delete and then recreate in separate
    # operations).  2021-09-01 JP.
    @tag :skip
    test "on conflict replace_all and conflict_target" do
      ...
      # Multiple record change value: note IDS are also replaced
      changes = [
        %{id: post_second.id + 1, title: "first_updated", visits: 1, uuid: post_first.uuid},
        %{id: post_second.id + 2, title: "second_updated", visits: 2, uuid: post_second.uuid}
      ]
