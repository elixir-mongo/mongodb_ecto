defmodule Ecto.Integration.TypeTest do
  ...
  test "array type" do
    ...
    # TODO I don't even know what this means, much less why it's a useful query at all.
    # It also doesn't work and I don't know how or why to fix it.
  ...
  # TODO FAIL
  # Need to implemeent :json_extract_path in the projection
  @tag :map_type
  ...
  # TODO Don't think we can implement this in Mongo? Not sure what union_all does yet
  ...
  test "schemaless calendar types" do
    datetime = ~N[2014-01-16 20:26:52]
    ...
    # TODO This should fail as nothing updated as it's set to the samething it was set to before
    # IO.inspect(TestRepo.update_all("posts", set: [inserted_at: datetime]))
    # Instead, this passes
    assert {1, _} = TestRepo.update_all("posts", set: [inserted_at: ~N[2014-01-16 20:26:53]])
  end
end
